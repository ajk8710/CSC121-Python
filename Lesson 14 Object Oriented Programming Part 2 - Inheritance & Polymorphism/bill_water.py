    ...
    def calculate_charge(self):
        ...
        while redo:
            try:
                ...
                if gallons < 0:
                    print('Gallons cannot be less than 0.')
                else:
                    if gallons <= 6000:
                        self._total = 0.005 * gallons
                    else:
                        self._total = 30 + 0.007 * (gallons - 6000)
                    self._gallons = gallons
                    redo = False
            except ValueError:
                print('Please enter numbers for gallons of water.')

    def display_bill(self):
        print(f'Water Bill\nName: {self._name}\nAddress: {self._address}\n' +
              f'Gallons Used: {self._gallons}\nPlease pay this amount: ${self._total:,.2f}')
